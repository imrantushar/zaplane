...
const ZAPInput = ({
  ...
  return <div style={{display:'flex', flexDirection:'column', gap:'8px', ...containerStyle}}>
      ...
      {isTextarea ? <textarea className="zaplane-textarea text-[var(--zaplane-text-muted)] bg-[var(--zaplane-background)]" placeholder={__(placeholder, "zaplane")} value={value} onChange={onChange} onKeyDown={onKeyDown} style={{...inputStyle}} /> :
