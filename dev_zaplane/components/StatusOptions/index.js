...
const StatusOptions = props => {
  ...
  useEffect(() => {
    ...
    } else if (relativeTo.current && relativeTo.current.parentNode === document.body) {
      document.body.removeChild(menuItemRef.current);
    }
  }, [isOpenDropdown, relativeTo]);
  return <>
			<button className={`zaplane-dropdown-option zaplane-dropdown-option--${value} 
				${suffix && 'zaplane-dropdown-option--' + suffix}`} type="button" ref={relativeTo} onClick={handleMenuToggle}>
				<span className="label">{statusLabel[value]}</span>
				<span>
					<FaAngleDown />
				</span>
			</button>
			{createPortal(<div className="zaplane-dropdown-option__lists" ref={menuItemRef}>
					{isOpenDropdown && <ul className="zaplane-more-status-options">
							{options.items.map((item, index) => <li className={`zaplane-more-status-options__item zaplane-more-status-options__item--${item.value} ${item.value === value && 'active'}`} role="presentation" onClick={() => {
