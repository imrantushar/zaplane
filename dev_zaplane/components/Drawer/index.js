...
const ZAPDrawer = ({
  ...
  const PanelContent = (
    <div className="flex h-full flex-col overflow-y-scroll bg-white shadow-xl pointer-events-auto">
      {title && (
        <div className="px-4 py-6 sm:px-6 flex items-center justify-between ">
          <div className="flex items-center">
            ...
            <h2 className="text-base font-semibold leading-6 text-gray-900 m-0">
  ...
  const DrawerTransition = (
    <div className={`fixed inset-y-0 ${placement === 'start' ? 'left-0' : 'right-0'} flex max-w-full ${isFullscreen ? '' : 'mt-8 '} ${!hasOverlay ? 'pointer-events-none' : ''}`} style={hasOverlay ? {} : { zIndex }}>
      <TransitionChild
        as={Fragment}
      ...
      >
        <div className={`w-screen ${isFullscreen ? 'max-w-full' : maxWidth}`}>
          {hasOverlay ? (
