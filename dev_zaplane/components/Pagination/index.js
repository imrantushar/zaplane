...
const Pagination = ({
	...
	const renderPageNumbers = () => {
		...
		if (pageNumbers.length <= 5) {
			...
			pageNumbers.forEach((item, index) => {
				const pageItem = (
					<li
						className={`zaplane-pagination-list__item ${item === page &&
							'zaplane-pagination-list__item-active'
							}`}
						key={index}
						onClick={() => {
			...
			for (let i = 1; i <= 3; i++) {
				const pageItem = (
					<li
						className={`zaplane-pagination-list__item ${i === page &&
							'zaplane-pagination-list__item-active'
							}`}
						key={i}
						onClick={() => {
			...
			renderedPages.push(
				<li
					className="zaplane-pagination-list__item zaplane-pagination-list__item-dots"
					key="dots"
				>
					...
				</li>
			...
			const lastPageItem = (
				<li
					className="zaplane-pagination-list__item"
					key={totalPages}
					onClick={() => {
	...
	return (
		<div className="zaplane-pagination">
			{showPrevButton && (
				<>
					<Button
